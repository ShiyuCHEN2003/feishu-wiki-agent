...
基于 Claude AI 的飞书（Lark）知识库自动化整理工具。
...
API 权限只控制接口调用范围，还需在知识库内为机器人授权。飞书知识库通过**添加群**的方式关联机器人，步骤如下：

1. 将机器人添加到一个飞书群（在群设置中添加机器人）
2. 打开目标知识库，点击右上角 **···** → **更多设置** → **成员管理**
3. 点击**添加群**，选择上一步中包含机器人的群
4. 权限设置为**管理员**
5. 保存

> 群消息相关权限（`im:chat:readonly` 等）是将机器人加入群时飞书平台的必要前提，非本 Agent 的业务功能。
